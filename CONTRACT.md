...
Status: draft v0.10. Shapes are stable enough to build against; additive fields may come, breaking changes go through the orchestrator.
...
- Every highway edge belongs to exactly one `streets.highwayStructures` record. Its ramps rise only at city-edge termini. Its 2 x 2 m supports stand on the centerline under the flat deck at no more than 30 m intervals, from ground to the deck underside. Decks keep 1 m of construction clearance beyond each edge from every building footprint, and support footprints never enter a building footprint. All support centers are on the 1 mm coordinate grid.
