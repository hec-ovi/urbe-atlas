...
- `prepareMasks({masks, encoding?})` in [PreparedMasks.ts](PreparedMasks.ts) takes [PartitionMaskInput](schema.ts) and returns an opaque [PreparedPartitionMasks](schema.ts) snapshot. Default encoding is binary. Geometry validates once; later changes to inputs or query results do not change the snapshot. A claim's optional `preparedMasks` unions that snapshot with its other masks, retaining its original encoding. Repeated queries reuse ring and edge indexes for local boundaries and exact horizontal-ray winding; the snapshot retains no query results.
...
- `edgeSupportEnclosures({from, to, encoding:'authored-1mm'})` in [EdgeSupportEnclosures.ts](EdgeSupportEnclosures.ts) returns `{from, to}` coordinate boxes. Exact affine interpolation of these boxes encloses every numeric `edgePositionView(t)` for binary `t` in `[0,1]`. Constant coordinates have zero-width boxes at their published value. For each varying coordinate, exact `h = max(abs(from), abs(to)) * 2^-53` bounds nearest-rounding error; endpoint boxes use tight floor/ceil of the authored coordinate minus/plus `h`. These diagnostic boxes do not move geometry.
...
- Numeric conversion rounds each rational coordinate once to the nearest Float64, with even midpoint ties and subnormal spacing. Exact quotient and remainder determine the rounding without intermediate integer conversion error.
