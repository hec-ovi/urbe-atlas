...
- `prepareMasks({masks, encoding?})` in [PreparedMasks.ts](PreparedMasks.ts) takes [PartitionMaskInput](schema.ts) and returns an opaque [PreparedPartitionMasks](schema.ts) snapshot. Default encoding is binary. Geometry validates once; later changes to inputs or query results do not change the snapshot. A claim's optional `preparedMasks` unions that snapshot with its other masks, retaining its original encoding. Repeated queries reuse exact boundary indexes and retain no query results in the snapshot.
