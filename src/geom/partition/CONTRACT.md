...
- `SourcePartition.create({id, source})` retains a finite simple source ring, in either winding.
...
- `finish()` returns shared numeric vertices, simple owner pieces and a nonpublishing certificate. Fixed pieces retain their reserved polygon and corner order verbatim. `verifyPartition({source, partition})` checks coverage, incidence and conversion before the caller discards the certificate.
