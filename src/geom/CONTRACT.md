...
- `numericSweepEnvelope(input)` in [NumericSweep.ts](NumericSweep.ts) takes two authored 1 mm source sides and returns their numeric-view envelope and per-side endpoint boxes, [types](schema.ts). The source endpoints must span positive area. Its convex CCW polygon encloses every published `edgePositionView(t)` on either side for binary `t` in `[0,1]`, and their convex hull. Returned boxes retain the same all-`t` guarantee for station projection. These copies are diagnostic data, not source geometry.
- `boxOf(points)`, `grow(box, by)`, `overlaps(a, b)` and `new BoxPairs(boxes).after(index)` in [Boxes.ts](Boxes.ts) answer axis-aligned rectangles with arithmetic. `BoxPairs` indexes a fixed set into uniform cells and returns, ascending and once each, every later rectangle whose box overlaps the one asked for, so a caller compares only pairs that can touch. It moves no geometry and holds no polygons.
...
- Numeric sweeps use the source partition's exact-authored, one-round support enclosures. Constant coordinates remain exact. The hull uses exact orientation and retains enclosure coordinates without another arithmetic expansion, snap or distance tolerance.

## Errors

`E_INVARIANT` if the normalized arrangement still contains a crossing, a diagnostic width/grid is not finite and positive, a coordinate-cover ring is non-finite, degenerate or closed by a repeated endpoint, or numeric-sweep sides are invalid or span no area.

## Depends on

- Root Atlas: coordinates and [polygon types](../../schema/blueprint.ts).
- Existing `clipper2-ts`: integer Boolean and offset operations.
- [Source partition](partition/CONTRACT.md): authored support enclosures for numeric-sweep diagnostics.
