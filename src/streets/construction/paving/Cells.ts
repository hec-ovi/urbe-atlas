...
export class Cells {
  static select(boundaries: Polygon[], frame: PavingFrame, module: PavingModule, covers: (polygon: Polygon) => boolean,
    accepts: (column: number, row: number) => boolean = () => true): FittedCells[] {
    const spans: Span[] = [];
    const visited = new Set<string>();
    for (const piece of boundaries) {
      const extent = bounds([piece.map(point => local(frame, point))]);
      const from = Math.floor(extent.min[0] / module.pitch[0]);
      const to = Math.ceil(extent.max[0] / module.pitch[0]);
      const bottom = Math.floor(extent.min[1] / module.pitch[1]);
      const top = Math.ceil(extent.max[1] / module.pitch[1]);
      for (let row = bottom; row < top; row++) {
        let start: number | undefined;
        for (let column = from; column < to; column++) {
          const footprint = cell(frame, module, column, row);
    ...
    return groups.map(cells => ({
      polygon: outline(frame, module, cells[0].from, cells[0].to, cells[0].row, cells[cells.length - 1].row + 1),
