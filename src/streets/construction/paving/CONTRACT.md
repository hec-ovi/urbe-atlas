...
- `PavingPlanner.plan(input)` in [PavingPlanner.ts](PavingPlanner.ts) takes standalone ground polygons, published street graph and runs, district boundaries, explicit paving settings and optional station bay footprints. This standalone polygon entry preserves each supplied binary source independently. It returns replacement ground and paving data; roadway, block and open records pass through unchanged.
...
- Functional bands remain independent of material role. A furnishing region contains actual furnishing land only. Shared junction and station-bay circulation is named separately. Returns and residuals retain their exact source boundaries.
...
- Straight curb residuals retain the module's U joint stations through their complete existing width. Full cells remain unchanged; residual bodies and joint strips partition only the retained curb owner. Joint masks use the frame's metre coordinates without snapping derived boundaries. Curved returns keep their explicit solid construction.
...
- Producer curb solids use curb or joint; border solids use border. Other bands allow body, joint, border and corner-infill; crossing-field and approach roles require circulation ownership.
