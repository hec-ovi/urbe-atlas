...
`DiagonalBlock.build(input)` takes [DiagonalBlockInput](schema.ts) and returns [DiagonalBlockTemplate](schema.ts): physical prisms, two buildable land regions, authored frontage/corner supports and the cut's line equation. Coordinates are local metres; the caller translates the template and joins the street graph.
...
The `planning` contours retain each original road-facing tangent span, inward normal, paved width and rounded corner arc. Their order matches the two interiors. These are construction supports; the physical beds retain ground ownership.
