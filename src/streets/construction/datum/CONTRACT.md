...
- `GradeDatum.clearanceFootprints(input)` takes a completed plan, later supports, ground top and caller-supplied clear height. It returns source-owned footprints whose solid geometry intersects that vertical range. Zero clear height queries contact at the ground datum.
...
- Frontage source tags name contributing spans through the source/result grid neighborhoods. This metadata correspondence changes no ground polygon and does not require one source to cover an entire combined frontage segment.
