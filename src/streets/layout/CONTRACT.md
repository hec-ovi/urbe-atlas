...
Optional `diagonals` defaults to true. Sparse interior blocks can carry one complete 30 or 45 degree street cut, with one or two lanes and fixed module junctions. Candidates have no parking bay and use local frontage roads. Their new junctions remain at least 40 m from the original street nodes; insufficient candidates leave the base grid intact. The target is one cut per 20 blocks, with at least one candidate considered. Cuts are nonadjacent and remain inside one block. Graph splits preserve source run stations and frontage references.
