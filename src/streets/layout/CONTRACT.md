...
The reserved rectangle remains inside the requested size. An optional perimeter profile constructs continuous outer sidewalks and supplies their directed edge sections. It requires whole-metre road spans. Optional `perimeter.exclusions` are land the ring may not stand on (water reaching the city boundary): the ring stops at that shoreline, publishing one frontage per remaining stretch of the side, and the land it vacates returns to the caller. The caller owns the remaining land outside the roadway and sidewalk rectangle. Native parking occurs on at most one 6 m paved frontage per selected block, with a 15% selection probability and complete clearances. Reserved highway frontages are ineligible. Bays have 6 by 2.5 m slots in a rectangular notch and 2 m support aprons; the complete support stays 6 m clear of corners. Output includes explicit bay and slot footprints and 3.5 m walking clearance. A selected side publishes a derived parking profile with walking interval 3..6.5 m from the road edge, over its existing rear sidewalk. Overall paved/side width and bay geometry remain fixed. Custom profiles requiring more than 3.5 m walking width are ineligible. Guardrails occupy 2, 4 or 6 m groups on sparse seeded frontages, at most two groups per block. Parking and rail choices use independent seed streams. Same seed, settings and side selection produce identical data.
