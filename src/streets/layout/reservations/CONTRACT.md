...
`city.modules.format` selects the dimension contract. Source construction, including omitted format, permits 2/4/6 m paved widths with 0.2 m curbs and 0.3 m gutters. District construction requires 4.2 m paved widths, 0.2 m curbs and 0.5 m gutters. Native parking has 6 m slots in a rectangular bay and exact published footprints: source bays are 2.5 m deep on 6 m paved frontages with 3.5 m walking clearance; district bays are 2 m deep with 2.2 m walking clearance. A bay footprint is exactly the roadway ground record its module notched into the kerb, on the same owner, one record per bay and no record without a bay. Slot area is measured in local coordinates with a one-square-millimetre numeric tolerance; containment uses the authored geometry grid.
