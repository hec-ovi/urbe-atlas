...
- Creation and visualization load their [WorkspaceForm](../cities/forms/schema.ts) documents concurrently from `/api/forms/creation` and `/api/forms/visualization`. Views iterate their documents. Supported widgets: heading, note, section, text, slider, select, choice, toggle, optional-number, action, action-row, presets, file, grid and layers. A control reads its first path and writes every path in `paths`. Submission is blocked, with the reason inline, while a slider value misses its `multipleOf` grid or a widget's `total` sum is unmet. Unknown widgets render a note.
...
- Creation displays the form beside the city catalog in a 70/30 split. The template dropdown applies Compact (600 m), City (1000 m) or Metro (3000 m) settings while preserving the seed. Waterfront offers None, Lagoon, River and Sea coast. Subway is the public transit option. The Streets section edits lane width per road class, the four sidewalk bands, crossing headroom and the paving finish, and states the dimensions the district modules fix. Generate city submits the form. Footprints are rectangles on the shared building grid.
...
- Generation covers the workspace with a native modal dialog and makes its background inert. The progress bar, stage count and phase label use completed server stages, not elapsed-time estimates. Under the phase the dialog names what that stage produces, from the published [stage list](../../schema/progress.ts); a phase the list does not carry shows none. It also states throughout what a finished blueprint contains and what it leaves to later stages, from the same source. Cancel deletes the server job; the modal stays locked until deletion is confirmed. A failed Cancel reports an inline reason and enables retry. Successful cancellation prevents pending status or blueprint responses from opening a city.
...
- New forms and templates choose owned copies of the form's paving default: maintained 1 m slab cells, optional 2 m groups, 12 mm joints, 1 m curb stations, 0.2 m curb width and 1 m border panels. They also choose its street default: 1/2/4 lanes at 4/7/14 m, a 4.2 m paved sidewalk of 1 m border, 1 m furnishing, 2 m walking and a 0.2 m frontage separator, a 0.2 m curb, a 0.5 m gutter and 2.5 m of crossing headroom. That design is what an omitted one resolves to, so an untouched form generates the same city as bare defaults. Supplied paving survives form edits exactly and omitted paving remains omitted; a supplied street design is what the street controls then edit. Module geometry belongs to the producer.
