...
describe('city HTTP catalog', () => {
  it('persists submissions, runs one job and serves exact completed JSON after a restart', async () => {
  ...
  it('stays readable while its real worker is CPU-bound, reports stages and deletes any city', async () => {
    const dataDir = await directory();
    const release = join(dataDir, 'release-worker');
    const app = await serve({ dataDir, workerUrl: new URL('./test.worker.mjs', import.meta.url) });
    const running = await (await app.post({ seed: release })).json() as CityRecord;
    const progress = await eventually(async () => (await app.request(`/api/cities/${running.id}`)).json() as Promise<CityRecord>,
    ...
    // reads and further submissions stay available while the worker holds the CPU
    const queued = await (await app.post({ seed: release })).json() as CityRecord;
    expect((await (await app.request('/api/cities')).json()).cities).toHaveLength(2);
    expect((await (await app.request(`/api/cities/${queued.id}`)).json()).status).toBe('queued');

    // cancelling a running city waits for the worker to exit and removes its directory
    ...
    expect((await app.request(`/api/cities/${running.id}`)).status).toBe(404);
    ...
    const ready = await status(app, queued.id, 'ready');
  ...
  it('resumes queued work after a restart, marks interrupted jobs failed and retains generation errors', async () => {
    const dataDir = await directory();
    const generator = new ControlledGenerator();
    const app = await serve({ dataDir, generator: generator.generate });
    ...
    const failing = await serve({ dataDir: await directory(), generator: async () => { throw failures.shift(); } });
    for (const code of ['E_INVALID_PARAMS', 'E_UNSATISFIABLE', 'E_INVARIANT', 'E_GENERATION']) {
      const record = await (await failing.post({ seed: code })).json() as CityRecord;
      const failed = await status(failing, record.id, 'failed');
      expect(failed.error?.code).toBe(code);
      if (code === 'E_INVALID_PARAMS') expect(failed.error?.details).toEqual({ field: 'features' });
    }
  });

  it('returns request, missing-city and storage errors, and refuses to start on a corrupt catalog', async () => {
    const dataDir = await directory();
    ...
    // an incomplete directory is not a catalog entry; an unreadable record fails startup
    const corrupt = await directory();
    const id = '00000000-0000-0000-0000-000000000000';
    await mkdir(join(corrupt, id));
    const empty = await serve({ dataDir: corrupt });
    expect((await (await empty.request('/api/cities')).json()).cities).toEqual([]);
    await empty.api.close();
    await writeFile(join(corrupt, id, 'record.json'), '{');
    await expect(createCityApi({ dataDir: corrupt })).rejects.toMatchObject({ code: 'E_STORAGE' });
    expect(await readFile(join(corrupt, id, 'record.json'), 'utf8')).toBe('{');
  });

  it('serves creation and visualization form documents', async () => {
    const app = await serve({ dataDir: await directory() });
    expect(await (await app.request('/api/forms')).json()).toEqual({ forms: ['creation', 'visualization'] });
