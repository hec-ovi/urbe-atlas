 ...
 */
import type { BuildingParcelType, Polygon } from '../../schema/blueprint';
...
const SETBACK: Partial<Record<BuildingParcelType, number>> = {
  residential: 2,
  factory: 3,
  military: 4,
  hospital: 3,
...
export function setback(type: BuildingParcelType): number {
  return SETBACK[type] ?? DEFAULT_SETBACK;
}

export function hostingProfile(type: BuildingParcelType): HostingProfile {
...
export function hostingProfiles(type: BuildingParcelType, fallback: BuildingParcelType): HostingProfile[] {
...
export function lotHosts(lot: Polygon, type: BuildingParcelType, host: FootprintHost): boolean {
