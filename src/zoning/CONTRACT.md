...
- A parcel's tier drifts one step around its district's, then steps down to mid when its lot is under 24 m on the short side: rich and high_rich need three 8 m bays. `RICH_MIN_SIDE` and `tierForSide` in [TierPolicy.ts](TierPolicy.ts) publish that bound.
